...
source("~/Documents/Practical machine learning/Capstone/final_project/ngramcreation.R")

#extracting bigrams,
data <- list(corpus = corpus)
data$txt <- readRDS(sprintf("%s/%s.train.clean.rds", data_clean, corpus))
...
data <- list(corpus = corpus)
data$txt <- readRDS(sprintf("%s/%s.train.clean.rds", data_clean.dir, corpus))
data$chunkSize <- 10000
data$lines <- length(data$txt)
data$ids <- initIds(data)
data$ng <- 4
