...
##data2[grep("Motor Vehicles",data2$SCC.Level.Three),"motortest"] <- "1"
#Coal_id<-uniqueN(data2coal,by=list(data2coal$SCC))
data_final<-subset(data,data$type=="ON-ROAD" & data$fips=="24510")

##merged_set<-merge(data,motor_id[,],by.x ="SCC",by.y="SCC")
Emission<-aggregate(data_final$Emissions,by=list(data_final$year),sum,na.rm=TRUE)
